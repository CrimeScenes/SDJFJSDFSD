...
-- Checks if a position is within the box FOV
...
-- Calculates predicted position based on velocity
...
-- Toggle and hold shooting modes
...
-- Input detection to start or stop shooting
UserInputService.InputBegan:Connect(function(input, gameProcessed)
...
-- Optional RenderStepped for 'hold' mode shooting
...
local function MostFavorable(player)
    local distanceScore = (player.Character.HumanoidRootPart.Position - Current_Camera.CFrame.Position).Magnitude
    local health = player.Character:FindFirstChildOfClass("Humanoid") and player.Character:FindFirstChildOfClass("Humanoid").Health or 100
    local movementScore = player.Character.HumanoidRootPart.Velocity.Magnitude
    return distanceScore - (health * 0.5) - movementScore
end

RunService.RenderStepped:Connect(UpdateFOV)

local function ClosestPlrFromMouse()
    local Target, ClosestScore = nil, math.huge
    ...
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if Circle.Radius > Distance and OnScreen then
                local score = MostFavorable(player)
...
-- Function to get most favorable body part of a character
        ...
        for _, part in ipairs(character:GetChildren()) do
            if part:IsA("BasePart") then
                ...
                if OnScreen then
                    ...
                    local score = Distance + (movementScore * 0.2) -- Adjust weight as needed

                    if Circle.Radius > Distance and score < BestScore then
                        BestScore = score
                        BestPart = part
                    end
                end
...
RunService.RenderStepped:Connect(function()
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        ...
        if Forbidden.CamLock.Normal.ClosestPart then
            BodyPart = GetMostFavorableBodyPart(TargetPlayer.Character)
        else
...
Run_Service.RenderStepped:Connect(function()
    if getgenv().Forbidden.Silent.Normal.Enabled and IsTargeting then  -- Only work when CamLock is engaged
        ...
        Target = ClosestPlrFromMouse()  -- Get the most favorable player instead of closest player
        ...
        if Target and Target.Character then
            ...
            local mostFavorablePart = GetMostFavorableBodyPart(Target.Character)  -- Get the most favorable body part
            if mostFavorablePart then
                local velocity = GetVelocity(Target, mostFavorablePart.Name)
                Replicated_Storage[RemoteEvent]:FireServer(Argument, mostFavorablePart.Position + velocity * getgenv().Forbidden.Silent.Normal.Prediction)
            end
        end
    else
...
task.spawn(function()
    while task.wait(0.1) do
        if getgenv().Forbidden.Silent.Normal.Enabled then
            Target = ClosestPlrFromMouse()  -- Update Target using most favorable player
